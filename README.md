...
material only. It must not override the level specifications.
...
The payload generated by the chat is the complete operation. The user shall not edit its contents unless explicitly instructed.

The execution evidence produced by the bootstrap executor is authoritative. The chat shall derive its conclusions from that evidence rather than from assumptions or command output.
