...
## Bootstrap Development Workflow Interaction Model
...
1. a downloadable JSON operation payload. The payload generated by the chat is the complete operation. The user should not edit its contents unless explicitly instructed.;
...
3. the expected path of the result artifact that the user should return to the chat for review. The execution evidence produced by the bootstrap executor is authoritative. The chat’s conclusions shall be derived from that evidence rather than from assumptions or command output.
