...
The next bounded implementation should reconcile planning and complete the
functional construction skeleton.

It should:

- replace the two prior implementation plans with this consolidated plan;
- preserve the identity scratchpad as evidence;
- correct encoding defects;
- add functional placeholders for every intended authority and validation area;
- extend the construction manifest to cover the complete skeleton;
- extend structural validation and tests;
- preserve all artifacts as non-normative and under construction;
- avoid substantive identity, repository, manifest, Level, source, or process
  semantics.

After that issue is accepted, the next issue should establish the generic
identity construction skeleton.
