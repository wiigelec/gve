...
def contained_path(root: Path, value: Any, label: str) -> Path:
    ...
    if set(part.lower() for part in pure.parts) & FORBIDDEN_NAME_PARTS:
...
def validate(root: Path) -> None:
    ...
    if manifest["construction_identity"] != (
        ...
        fail(
            "GVE-RSC-IDENTITY-002",
            f"{MANIFEST_PATH}: unexpected construction identity",
        )
    if manifest["validation_entry_point"] != "validate":
        fail(
            "GVE-RSC-PATH-004",
            f"{MANIFEST_PATH}: validation entry point must be 'validate'",
        )
    classes = string_list(
    ...
    if paths != list(PLACEHOLDER_PATHS):
        fail(
            "GVE-RSC-PATH-004",
            f"{MANIFEST_PATH}: artifact paths do not match bounded inventory",
        )
    for item in paths:
        contained_path(root, item, f"{MANIFEST_PATH}.artifact_paths")
    ...
    for relative in PLACEHOLDER_PATHS:
        ...
        if identity in identities:
            fail(
                "GVE-RSC-IDENTITY-003",
                f"{relative}: duplicate construction identity {identity!r}",
            )
        identities.add(identity)
        if not isinstance(value["responsibility"], str) or not value[
            "responsibility"
        ].strip():
            fail(
                "GVE-RSC-TYPE-001",
                f"{relative}.responsibility: must be a non-empty string",
            )
        string_list(
            value["expected_relationships"],
            f"{relative}.expected_relationships",
        )
